...
output_csv = "/scratch/gpfs/bb4178/SEP_prediction/data_collection/SHARP_to_SolO_times.csv"
...
def find_SHARP_for_SolO_time(target_time, SHARP_times, window_hours=1):
    '''
    Find the indices of SHARP measurements within a window around a given SolO time.
...
def test_find_SHARP_for_SolO_time():
    '''
    Test the find_SHARP_for_SolO_time function with various cases.
    '''
    ...
    bottom_index, top_index = find_SHARP_for_SolO_time(target_time, SHARP_times, window_hours)
    ...
    print("All tests passed for find_SHARP_for_SolO_time!")

test_find_SHARP_for_SolO_time()
...
# Find the mean SHARP measurement for each SolO time and store the results in a csv file
results = []

times = df_solO["time"].to_numpy(dtype="datetime64[ns]")
linlin = df_solO["linlin"].to_numpy(dtype=np.float64)
smoothed_linlin = [compute_mean_flux(times, linlin, time) for time in times]
count = 0

for offset_SolO_time, SolO_time, Jlinlin, footprint_valid, lon, r, footprint_lon in tqdm(df_solO[["offset_time", "time", "linlin", "footprint_valid", "hgs_lon", "spacecraft_r", "solo_footpoint_hgs_lon"]].values, desc="Processing SolO times"):
    ...
    # Skip if the angular separation between the SolO footpoint and Earth is too large, since the SHARP measurements may not be relevant for this SolO time
    if not footprint_valid:
        continue

    bottom_index, top_index = find_SHARP_for_SolO_time(offset_SolO_time, SHARP_times, cadence_hours/2)
    ...
    # Maps ARPNUM to the SHARP measurements for that active region in the window around the SolO time. If there are no SHARP measurements for an active region in the window, it will not be included in the dictionary.
    AR_dict = {}
    ...
    for ARPNUM, measurements in AR_dict.items():
        ...
        result_dict = {"time": SolO_time, "Jlinlin": compute_mean_flux_cadence(times, smoothed_linlin, SolO_time, cadence_hours), "Jlinlin_raw": Jlinlin, "ARPNUM": ARPNUM, "hgs_lon": lon, "spacecraft_r": r, "footprint_lon": footprint_lon}
        for col in SHARP_columns:
