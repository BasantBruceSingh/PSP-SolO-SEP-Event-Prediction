...
for i in range(10):
    ...
    # Save outputs
    # -----------------------------
    np.save(os.path.join(OUT_DIR, f"X_t_train_combined_na_{i}.npy"), X_train)
    np.save(os.path.join(OUT_DIR, f"X_t_val_combined_na_{i}.npy"), X_val)
    np.save(os.path.join(OUT_DIR, f"X_t_test_combined_na_{i}.npy"), X_test)

    np.save(os.path.join(OUT_DIR, f"mask_t_train_combined_na_{i}.npy"), mask_train)
    np.save(os.path.join(OUT_DIR, f"mask_t_val_combined_na_{i}.npy"), mask_val)
    np.save(os.path.join(OUT_DIR, f"mask_t_test_combined_na_{i}.npy"), mask_test)

    np.save(os.path.join(OUT_DIR, f"y_t_train_combined_na_{i}.npy"), y_train)
    np.save(os.path.join(OUT_DIR, f"y_t_val_combined_na_{i}.npy"), y_val)
    np.save(os.path.join(OUT_DIR, f"y_t_test_combined_na_{i}.npy"), y_test)

    np.save(os.path.join(OUT_DIR, f"sat_t_train_combined_na_{i}.npy"), sat_train)
    np.save(os.path.join(OUT_DIR, f"sat_t_val_combined_na_{i}.npy"), sat_val)
    np.save(os.path.join(OUT_DIR, f"sat_t_test_combined_na_{i}.npy"), sat_test)

    pd.DataFrame(y_stats).T.to_csv(
        os.path.join(OUT_DIR, f"y_point_normalization_stats_by_satellite_na_{i}.csv")
    )
